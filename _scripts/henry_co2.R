...
partition <- function(t_atm, alk, tco2, sal = 35) {
  ...
  k1 = 13.7201 - 0.031334 * t_kt - 3235.76 / t_kt - 1.3E-5 * sal * t_kt +
    0.1032 * sqrt(sal)
...
#
co2_saturation <- function(co2_atmos, alkalinity_ocean, co2_total, temp_atmos) {
  # Note: the TCO2 and ALK columns in the Chicago model are transposed, so fix them here
  #  correct_alk = alk # tco2
  #  correct_tco2 = tco2 # alk
  if (is.vector(co2_atmos)) {
    vapor_pressure = pmap(list(temp_atmos, alkalinity_ocean, co2_total),
                          partition) %>%
      map_dbl(~.x['pco2'])
  } else {
    part = partition(t_atm = temp_atmos, alk = alkalinity_ocean,
                     tco2 = co2_total)
    vapor_pressure = part['pco2']
  }
  vapor_pressure / co2_atmos
}
